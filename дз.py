...
print('№4')
print('False. Пример: bool(False) = False.
 0. Пример: bool(0) = False. Пустые последовательности (строки, списки, кортежи и т. д.). 1 Примеры: bool('') = False, bool() [] = False, bool({}) = False.')))

print('№5')   
print('потому что человеку, читающему такой код, будет сложно понять порядок вычисления значений.')

print('#6')
print('Выражение «a and b» возвращает последнее истинное значение или первое ложное (остальные выражения даже не будут вычисляться в этом случае). 1
Выражение «bool(a) and bool(b)» всегда вернёт булево значение (то есть True или False). ')

print('№7')
print('В первую очередь рассчитываются выражения с оператором not. 
    Далее вычисляются все конструкции, имеющие оператор and. 
    В последнюю очередь работает оператор or.')
